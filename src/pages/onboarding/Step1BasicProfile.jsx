...
export default function Step1BasicProfile() {
  ...
  const validateStep1 = () => {
    ...
    if (
      ...
      formData.investmentPreferences.minAmount === null
    ) {
    ...
    if (
      ...
      formData.investmentPreferences.maxAmount === null
    ) {
      newErrors.maxAmount = "Maximum amount is required.";
    }

    const min = Number(formData.investmentPreferences.minAmount);
    const max = Number(formData.investmentPreferences.maxAmount);

    if (
      formData.investmentPreferences.minAmount !== "" &&
      Number.isNaN(min)
    ) {
      newErrors.minAmount = "Minimum amount must be a valid number.";
    }
    ...
    if (
      ...
      newErrors.maxAmount = "Maximum amount must be a valid number.";
    }

    if (!newErrors.minAmount && min < 0) {
      newErrors.minAmount = "Minimum amount cannot be negative.";
    }

    if (!newErrors.maxAmount && max < 0) {
    ...
    if (!formData.investmentPreferences.currency) {
  ...
  const handleContinue = (e) => {
    ...
    // On garde le draft en localStorage.
      ...
      <div className="flex pt-16 min-h-screen">
        <aside className="hidden lg:flex h-[calc(100vh-64px)] w-72 fixed left-0 flex-col gap-1 p-4 bg-slate-50 border-r border-slate-200/50">
          ...
          <nav className="space-y-1">
            <Link className="flex items-center gap-3 px-4 py-3 bg-white text-slate-900 font-bold shadow-sm rounded-xl transition-all duration-200 ease-in-out" to="/Step1BasicProfile">
            ...
            <div className="flex items-center gap-3 px-4 py-3 text-slate-400 rounded-xl transition-all duration-200 ease-in-out cursor-not-allowed">
  ...
  <div className="mb-6">
    ...
    {errors.investorType && (
      <p className="text-sm text-error mt-2">{errors.investorType}</p>
    )}
  </div>
...
<section>
  <div className="mb-6">
    ...
    {errors.riskTolerance && (
      <p className="text-sm text-error mt-2">{errors.riskTolerance}</p>
    )}
  </div>
  ...
  <div className="relative group max-w-xl">
    ...
    >
      ...
      <option value="MEDIUM">MEDIUM — Balanced Growth & Stability</option>
      <option value="HIGH">HIGH — Aggressive Growth Search</option>
        ...
        expand_more
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="relative">
                        <input
                          type="number"
                          placeholder="0.00"
                          value={formData.investmentPreferences.minAmount}
                          onChange={(e) =>
                      ...
                      {errors.minAmount && (
                        <p className="text-sm text-error">{errors.minAmount}</p>
                      )}
                    </div>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="relative">
                        <input
                          type="number"
                          placeholder="0.00"
                          value={formData.investmentPreferences.maxAmount}
                          onChange={(e) =>
                            handleInvestmentPreferenceChange(
                              "maxAmount",
                              e.target.value
                      ...
                      {errors.maxAmount && (
                        <p className="text-sm text-error">{errors.maxAmount}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <footer className="mt-16 pt-8 border-t border-outline-variant/10 flex justify-between items-center">
              ...
              <div className="flex items-center justify-between pt-4">
                <button
                  type="submit"
                  onClick={() => navigate("/Step2DetailsClassification")}
